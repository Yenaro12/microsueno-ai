...
const CENTRO_INICIAL = [19.4326, -99.1332]

const formatoTiempo = (milisegundos) => `${((milisegundos || 0) / 1000).toFixed(1)} s`
...
function ControlCentrado({ ubicacionActual, centrarMapaToken }) {
  ...
  useEffect(() => {
    ...
    mapa.setView([ubicacionActual.latitud, ubicacionActual.longitud], 16, {
      animate: true,
    })
...
function RouteMap({ ubicacionActual, rutaRecorrida, eventos, centrarMapaToken }) {
  ...
  return (
    <MapContainer center={centro} zoom={14} scrollWheelZoom className="mapa-ruta">
      ...
      />
      {ruta.length > 1 && (
        <Polyline positions={ruta} pathOptions={{ color: '#0f766e', weight: 5 }} />
      )}
      {ubicacionActual && (
        <CircleMarker
      ...
      {eventosConUbicacion.map((evento) => (
        ...
        >
          <Popup>
            ...
            Riesgo: {evento.nivel}
            <br />
            Duracion: {formatoTiempo(evento.duracionMs)}
            <br />
            Desplazamiento: {Number(evento.desplazamiento || 0).toFixed(1)} px
          </Popup>
        </CircleMarker>
      ))}
    </MapContainer>
